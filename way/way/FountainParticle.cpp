...
namespace FountainParticle {
	...
	FountainParticle::FountainParticle()
	{
		...
		mUpdateShader = new Shader("shaders/FountainUpdate.vs", "shaders/FountainUpdate.fs",
			"shaders/FountainUpdate.gs", varyings, 5);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new Shader("shaders/FountainRender.vs", "shaders/FountainRender.fs");
		//设置随机纹理
	...
	bool FountainParticle::InitFountain()
	{
	...
	void FountainParticle::Render(float frametimeMills, glm::mat4& worldMatrix,
	...
	void FountainParticle::UpdateParticles(float frametimeMills)
	{
		mUpdateShader->use();
	...
	void FountainParticle::RenderParticles(glm::mat4& worldMatrix,
		glm::mat4& viewMatrix, glm::mat4& projectMatrix)
	{
	...
	void FountainParticle::InitRandomTexture(unsigned int size)
	{
		srand(time(NULL));
	...
	void FountainParticle::GenInitLocation(WaterParticle particles[], int nums) {
		srand(time(NULL));
